    ...
    def update(self):
        ...
        if self.pump_H2_on:
            if self.level_K1 < 15.0:
                ...
                self.alarms.append(f"⚠️ КАВИТАЦИЯ Н-2! Вибрация растет: {self.vib_H2:.1f} мм/с")
            else:
        ...
        if self.vib_H2 > 9.0 and self.pump_H2_on:
            self.alarms.append("[ПАЗ] Критическая вибрация! Разрушение подшипников Н-2.")
            self.pump_H2_on = False
            self.score -= 20

        if self.level_K1 >= 100.0:
            self.alarms.append("🚫 ЗАХЛЕБЫВАНИЕ К-1! Унос жидкости в шлемовую трубу. Брак!")
            self.score -= 10

        if self.gas_loss:
            self.valve_gas = 0.0
            self.alarms.append("❗️ ОБРЫВ ПЛАМЕНИ П-3! Нет давления в сети топливного газа.")
        ...
        if flow_out == 0 and heat_in > 0:
            target_temp = 900.0
            self.alarms.append("🔥 ОПАСНОСТЬ ПРОГАРА ТРУБ П-3! Нет циркуляции сырья.")
