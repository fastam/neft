...
class PIDController:
    def __init__(self, kp, ki, out_min=0.0, out_max=100.0):
        self.kp = kp; self.ki = ki; self.out_min = out_min; self.out_max = out_max
        self.integral = 0.0
...
class SimulationEngine:
    def __init__(self):
        self.pump_H1_on = True; self.valve_feed = 80.0
        self.pump_H2_on = True
        self.auto_mode = True; self.sp_temp = 335.0; self.pid = PIDController(kp=2.5, ki=0.5); self.valve_gas = 83.5
        self.pcv_221 = 40.0; self.avz_1 = 80.0
        self.avz_broken = False; self.pcv_stuck = False; self.gas_stuck = False
        self.level_K1 = 50.0; self.temp_p3_out = 335.0; self.pressure_K1 = 2.5; self.temp_top_K1 = 140.0
        self.alarms = []; self.score = 100; self.exploded = False
    ...
    def update(self):
        ...
        flow_out = 125.0 if self.pump_H2_on else 0.0
        ...
        if self.level_K1 <= 5.0 and self.pump_H2_on:
            ...
            self.pump_H2_on = False; self.score -= 10
        ...
        target_temp = 240.0 + (heat_in / (flow_out * 2.1)) if flow_out > 0 else 900.0
        self.temp_p3_out += (target_temp - self.temp_p3_out) / 4.0
        
        vapor_gen = max(0, (self.temp_p3_out - 200) * 0.02)
        relief = (self.pcv_221 / 100.0) * 4.0
        ...
        self.temp_top_K1 += (max(20.0, self.temp_p3_out * 0.45 - cooling) - self.temp_top_K1) / 3.0
        ...
        if self.pressure_K1 > 4.5:
            ...
            if self.pressure_K1 > 5.2:
                ...
                self.exploded = True; self.score = 0
                
        if self.temp_top_K1 > 150.0: self.alarms.append("[СИГНАЛИЗАЦИЯ] Перегрев верха К-1 (>150°C)"); self.score -= 2
        if self.temp_p3_out >= 360.0: self.alarms.append("🔥 КРИТИЧЕСКАЯ АВАРИЯ: ПРОГАР ПЕЧИ П-3!"); self.score -= 10
