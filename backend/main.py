...
async def startup_event(): asyncio.create_task(simulation_loop())

@app.get("/api/state")
async def get_state():
    return {
        "score": engine.score, "alarms": engine.alarms, "exploded": engine.exploded,
        "pump_H1": engine.pump_H1_on, "valve_feed": round(engine.valve_feed, 1),
        "flow_in": round(150.0 * (engine.valve_feed / 100.0) if engine.pump_H1_on else 0, 1),
        "level_K1": round(engine.level_K1, 1), "pressure_K1": round(engine.pressure_K1, 2), "temp_top_K1": round(engine.temp_top_K1, 1),
        "pump_H2": engine.pump_H2_on, "flow_out": 125.0 if engine.pump_H2_on else 0.0,
        "TRC3_mode": "AUTO" if engine.auto_mode else "MANUAL", "valve_gas": round(engine.valve_gas, 1), "temp_P3": round(engine.temp_p3_out, 1),
        "pcv_221": round(engine.pcv_221, 1), "avz_1": round(engine.avz_1, 1),
        "avz_broken": engine.avz_broken, "pcv_stuck": engine.pcv_stuck, "gas_stuck": engine.gas_stuck
    }
